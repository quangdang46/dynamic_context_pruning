...
use crate::enums::{CacheStabilityMode, CacheStabilityModeSchema, InjectionMode, NudgeForce};
...
pub struct Config {
    ...
    /// How nudges are glued onto messages (SPEC.md §8 / §10.2).
    /// Defaults to `wrap_block`.
    pub injection_mode: InjectionMode,
    /// Tokenizer configuration.
    pub tokenizer: TokenizerConfig,
    /// Persistence configuration (SPEC.md §9.3).
...
impl Config {
    ...
    /// Configured image token cost.
    pub fn image_tokens(&self) -> u32 {
        self.tokenizer.image_tokens
    }
}
...
impl PartialEq for Config {
    fn eq(&self, other: &Self) -> bool {
        ...
        self.enabled == other.enabled
            ...
            && self.injection_mode == other.injection_mode
...
pub fn validate(config: &Config) -> Result<(), ConfigError> {
    ...
    // tokenizer.imageTokens in 1..=10000
    if config.tokenizer.image_tokens < 1 || config.tokenizer.image_tokens > 10000 {
        return Err(ConfigError::validation(format!(
            "tokenizer.imageTokens must be in 1..=10000 (was {})",
            config.tokenizer.image_tokens
        )));
    }

    // persistence.autoSaveSeconds in 0..=3600
    if config.persistence.auto_save_seconds > 3600 {
        return Err(ConfigError::validation(format!(
            "persistence.autoSaveSeconds must be in 0..=3600 (was {})",
            config.persistence.auto_save_seconds
        )));
    }
...
impl dcp_compress::CompressConfig for Config {
    ...
    fn protect_tags(&self) -> bool {
        self.compress.protect_tags
    }
    fn summary_buffer(&self) -> bool {
        self.compress.summary_buffer
    }
}
...
mod tests {
    ...
    fn default_matches_spec_field_by_field() {
        ...
        assert_eq!(c.tokenizer, TokenizerConfig::default());

        assert_eq!(c.strategies, StrategiesConfig::default());
        assert_eq!(c.commands, CommandsConfig::default());
        assert_eq!(c.experimental, ExperimentalConfig::default());
        assert_eq!(c.persistence, PersistenceConfig::default());
    }
    ...
    fn compress_config_accessors_match_spec() {
        ...
        assert!(!CC::protect_tags(&c));
        assert!(CC::summary_buffer(&c));
    }
    ...
    #[test]
    fn validate_rejects_image_tokens_below_1() {
        let mut c = Config::default();
        c.tokenizer.image_tokens = 0;
        let err = c.validate().unwrap_err();
        assert!(matches!(err, ConfigError::Validation(m) if m.contains("imageTokens")));
    }

    #[test]
    fn validate_rejects_image_tokens_over_10000() {
        let mut c = Config::default();
        c.tokenizer.image_tokens = 10001;
        let err = c.validate().unwrap_err();
        assert!(matches!(err, ConfigError::Validation(m) if m.contains("imageTokens")));
    }

    #[test]
    fn image_tokens_accessor_matches_config() {
        let c = Config::default();
        assert_eq!(c.image_tokens(), 1500);
        let mut c2 = Config::default();
        c2.tokenizer.image_tokens = 2000;
        assert_eq!(c2.image_tokens(), 2000);
    }
