...
/// (SPEC.md §10.2 — see also [`dcp_nudges::InjectionMode`]).
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Hash, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "snake_case")]
pub enum InjectionMode {
    /// Replace the message body with the rendered nudge.
    Replace,
    /// Append the nudge after the existing body. Default.
    #[default]
    Append,
    /// Prepend the nudge before the existing body.
    Prepend,
}
...
impl From<InjectionMode> for dcp_nudges::InjectionMode {
    fn from(value: InjectionMode) -> Self {
        match value {
            InjectionMode::Replace => Self::Replace,
            InjectionMode::Append => Self::Append,
            InjectionMode::Prepend => Self::Prepend,
        }
    }
}
...
impl From<dcp_nudges::InjectionMode> for InjectionMode {
    fn from(value: dcp_nudges::InjectionMode) -> Self {
        match value {
            dcp_nudges::InjectionMode::Replace => Self::Replace,
            dcp_nudges::InjectionMode::Append => Self::Append,
            dcp_nudges::InjectionMode::Prepend => Self::Prepend,
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn injection_mode_serde_snake_case() {
        assert_eq!(
            serde_json::to_string(&InjectionMode::Append).unwrap(),
            "\"append\""
        );
        assert_eq!(
            serde_json::to_string(&InjectionMode::Replace).unwrap(),
            "\"replace\""
        );
        assert_eq!(
            serde_json::to_string(&InjectionMode::Prepend).unwrap(),
            "\"prepend\""
        );
    }
